...
`node` 就是我们 DFS 到的当前节点，`content` 是 DFS 过程中的上下文，这个上下文贯穿始终，可以在任何节点修改，而 `children` 就是该节点所有子节点的渲染结果

那么假如我要处理一个加粗的标签，那么我可以这么做：

```js
{
  render(node: TagNode, content: IContent, children: string[]): string {
...
{
  enter(node: TagNode, content: IContent): void {
    if (content.cnt > content.max) {
      content.max = content.cnt
    }
    content.cnt++
  }
...
这样遍历完成之后，`content.max` 就是我们想知道的最深嵌套层数了

当然这是示例，实际上，这两个钩子的真正用处体现在以下两点:

1. 明确的让你知道自己的解析过程到达了什么位置

2. 让你能够去直接修改 AST 或者通过改变 `content` 来改变渲染行为
...
2. 未闭合的标签我们会按照顺序尝试匹配 `generalTagHandlers 中的 `handler`
